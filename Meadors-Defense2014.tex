...
\titlegraphic{\includegraphics[width=0.5\paperwidth,height=0.4\paperheight]{plots/ScoX1ULs.png}}
...
\date{2014 October 24~\includegraphics[height=0.3cm, width=1.9cm]{plots/michigan.png}}
...
--Can \textit{waves of gravity} be sensed?
\end{exampleblock}

\lyxframeend{}\lyxframe{Observing beyond light}

\textbf{General relativity}: extremize \textit{curvature} $R$,\newline
...
plexiglass arms, aluminum baseboard, green fringes w/ photodiode to listen\\
\textit{LIGO in miniature -- how can one enhance the real observatory?}
}
\end{figure}
...
\begin{figure}
\caption{\protect\includegraphics[width=0.88\paperwidth,height=0.5\paperheight]{plots/figure10.png}}
...
Most dramatic H1 filtering (green before, blue after)
...
template heatmap: modulation depth vs frequency
\end{figure}
...
Circular effective $h_0 = h_{0,\textup{ reported}} [( [(1+\cos^2 \iota)/2]^2 + [\cos \iota]^2)/2]^{1/2}$
...
Open-pulsar estimation of $f$, plotted vs $f$, ($a \sin i$, $h_0$ similar)\\
error sigma based on all detected open injections
\end{figure}
...
\begin{figure}
\caption{\protect\includegraphics[width=0.9\paperwidth,height=0.76\paperheight]{plots/ScoX1ULs.png}}
H1: loudest $h_0 \times \left( 1 + 1.0 \right) \times \left[\cos \iota \textup{ factor}\right]$ in 0.1 Hz bands
\end{figure}
...
\begin{description}
\item[{Search frequencies}] [434.5,436.5], [620.5,622.5] \& [869.5,871.5] Hz
\end{description}
...
(Just implemented by E. Goetz, soon to be tested)
...
\begin{itemize}
\item TwoSpect `won' Scorpius X-1 mock data challenge
\item S6 Scorpius X-1 and J1751-305 search processing done
\item Directed binary searches: becoming more sensitive
\end{itemize}

\emph{LIGO in the future}
\begin{itemize}
\item Advanced LIGO now being commissioned
\item Author has done filtering, squeezing to enhance LIGO, \\
  methods for finding low mass X-ray binaries
\item Coming soon: limits for Scorpius X-1, then \\
  even better for Advanced LIGO's first run in 2015
\end{itemize}
...
beginning TwoSpect, Keita Kawabe for launching
the filtering project, everyone on the squeezing
project, Chris Messenger for
leading the Scorpius X-1 MDC, 
\\ AEI for current \& future support and computing, and the \\
LVC-Virgo Collaboration \& National Science Foundation.
